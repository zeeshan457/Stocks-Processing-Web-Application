...
public class Validation {
    ...
    public boolean RegisterValidation(String username, String password1, String password2) {
        ...
        if (username.trim().isEmpty() || password1.trim().isEmpty() || password2.trim().isEmpty()) {
            ...
            Notification error = Notification.show("The username must more than 5 characters");
            error.addThemeVariants(NotificationVariant.LUMO_ERROR);
            return false;
        } else if (password1.length() < 5) {
            Notification error = Notification.show("The password must more than 5 characters");
            error.addThemeVariants(NotificationVariant.LUMO_ERROR);
            return false;
        }
        return true;
